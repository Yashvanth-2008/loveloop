...
function repeatMessage() {
    ...
    for (let i = 1; i <= count; i++) {
        ...
        // Store message WITH NUMBER for copy all functionality
        allMessages.push(`${i}. ${message}`);
    ...
    successMsg.innerHTML = `👑 Successfully generated ${count} royal message${count > 1 ? 's' : ''}! 👑`;
...
function copyAllMessages() {
    ...
    // Join with newline - already formatted with numbers
    ...
    navigator.clipboard.writeText(allText).then(() => {
        ...
        showToast(`👑 All ${allMessages.length} message${allMessages.length > 1 ? 's' : ''} copied!`);
